...
function plot_mean(df)
    ...
    t)
    capttext = "Figure shows the estimated model probability of being in each state for each date in the sample, conditional on all data through that date."
...
function plot_std(df)
    ...
    t)
    capttext = "Figure shows the estimated standard deviation of inflation conditional on being in each state, estimated conditional all data through each date, along\nwith the inflation rate."
    composition = compose(context(),
        (context(0.0w + 0.25inch, 0.0h + 0.25inch, 1.0w - 0.5inch, 1.0h - 1inch - 0.5inch), render(fig)),
        (context(0.0w, 1.0h - 1inch, 1.0w, 1inch), Compose.text(0.25inch, 0.0, capttext, hleft, vtop), fontsize(10pt), font("Helvetica")))
    return composition
...
function plot_forecast_error(df)
    ...
    t)
    capttext = "Figure shows the 12-month ahead forecast estimated using data through each date, along with the realized 12-month ahead inflation rate."
    composition = compose(context(),
        (context(0.0w + 0.25inch, 0.0h + 0.25inch, 1.0w - 0.5inch, 1.0h - 1inch - 0.5inch), render(fig)),
        (context(0.0w, 1.0h - 1inch, 1.0w, 1inch), Compose.text(0.25inch, 0.0, capttext, hleft, vtop), fontsize(10pt), font("Helvetica")))
    return composition
...
vstack(plot_mean(df1), plot_std(df2), plot_forecast(df4), plot_forecast_error(df4), plot_state_probs(df3)) |> PDF("plots/output/official_data.pdf", 10inch, 50inch)
...
vstack(plot_mean(df1), plot_std(df2), plot_forecast(df4), plot_forecast_error(df4), plot_state_probs(df3)) |> PDF("plots/output/official_data.pdf")
...
vstack(plot_mean(df1), plot_std(df2), plot_forecast(df4), plot_forecast_error(df4), plot_state_probs(df3)) |> PDF("plots/output/signals_official_noise_0.1_allsignal_data_.pdf", 10inch, 50inch)
...
vstack(plot_mean(df1), plot_std(df2), plot_forecast(df4), plot_forecast_error(df4), plot_state_probs(df3)) |> PDF("plots/output/signals_official_noise_0.3_allsignal_data_.pdf", 10inch, 50inch)
...
vstack(plot_mean(df1), plot_std(df2), plot_forecast(df4), plot_forecast_error(df4), plot_state_probs(df3)) |> PDF("plots/output/signals_official_noise_0.6_allsignal_data_.pdf", 10inch, 50inch)